...
class Solution:
    def xorQueries(self, arr: List[int], queries: List[List[int]]) -> List[int]:
        st = SegmentTree(arr, merger_=lambda x, y: x ^ y)  # 这里以求和为例
        ret = []
        for q in queries:
            ret.append(st.query(q[0], q[1]))
        return ret


if __name__ == "__main__":
    s = Solution()
    print(s.xorQueries([1, 3, 4, 8], queries=[[0, 1], [1, 2], [0, 3], [3, 3]]))  # [2,7,14,8]
    print(s.xorQueries([4, 8, 2, 10], queries=[[2, 3], [1, 3], [0, 0], [0, 3]]))  # [8,0,4,4]
